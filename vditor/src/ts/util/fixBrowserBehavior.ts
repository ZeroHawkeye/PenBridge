...
// 处理对齐指令块的键盘事件
export const fixAlignDirective = (vditor: IVditor, event: KeyboardEvent, alignDirectiveElement: HTMLElement, range: Range) => {
    // Backspace: 光标位于块的开始位置，删除整个对齐指令块
    if (event.key === "Backspace" && !isCtrl(event) && !event.shiftKey && !event.altKey) {
        // 获取块内所有段落的内容
        const paragraphs = alignDirectiveElement.querySelectorAll("p");
        let content = "";
        ...
        // 检查光标位置
        const position = getSelectPosition(alignDirectiveElement, vditor[vditor.currentMode].element, range);

        // 如果块内容为空或者光标在块的开始位置
        if (content.replace(Constants.ZWSP, "").trim() === "" || position.start <= 1) {
            // 将内容提取出来，删除对齐指令块
            if (content.trim() === "" || content === Constants.ZWSP) {
                alignDirectiveElement.outerHTML = `<p data-block="0"><wbr>\n</p>`;
            } else {
                alignDirectiveElement.outerHTML = `<p data-block="0"><wbr>${content}</p>`;
            }
            setRangeByWbr(vditor[vditor.currentMode].element, range);
            execAfterRender(vditor);
            event.preventDefault();
    ...
    if (event.key === "Enter" && !isCtrl(event) && !event.shiftKey && !event.altKey) {
        ...
        if (pElement && pElement.parentElement === alignDirectiveElement) {
            ...
            if (pContent.replace(Constants.ZWSP, "").trim() === "" && pElement.previousElementSibling) {
                // 检查前一个元素是否也是空段落
                const prevP = pElement.previousElementSibling;
                if (prevP.tagName === "P") {
                    // 在对齐指令块后添加新段落并跳出
